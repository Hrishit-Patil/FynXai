...
export default function ApplicationDetail() {
  ...
  const fetchApplicationDetails = async () => {
    try {
      ...
      const xaiData =
        xaiResponse && xaiResponse.length > 0 ? xaiResponse[0] : null;

      const documents = loanData.document_urls
        ? Object.entries(loanData.document_urls).map(([key, path]) => ({
      ...
      const employerName =
        firstPayslip.employer_name || bankData.employer_name || "N/A";
      ...
      const transformedApp: ApplicationData = {
        ...
        applicant: {
          ...
          aadhaar: "****-****-5678",
          address:
            ocrJson?.aadhaar_card?.back?.address || "Address not detected",
          employment: firstPayslip.employer_category || "Salaried",
          employer: employerName,
          experience: ocrJson?.employment_info?.employment_tenure_years
            ? `${ocrJson.employment_info.employment_tenure_years} years`
            : "N/A",
          ...
          emi:
            Math.round(
              (loanData.loan_amount *
                (10.5 / 1200) *
                Math.pow(1 + 10.5 / 1200, loanData.loan_tenure)) /
                (Math.pow(1 + 10.5 / 1200, loanData.loan_tenure) - 1),
            ) || 0,
        },
        documents: documents,
        ocrData: {
          name: {
            value: panData.name || "N/A",
            confidence: 0.98,
            verified: true,
          },
          pan: {
            value: panData.pan_number || loanData.pan_number,
            confidence: 0.99,
            verified: true,
          },
          salary: {
            value: String(loanData.monthly_income),
            confidence: 0.85,
            verified: false,
          },
          employer: { value: employerName, confidence: 0.88, verified: true },
          bankBalance: {
            value: String(monthEndBalance),
            confidence: 0.92,
            verified: true,
          },
        },
        ...
        creditScore: {
          ...
          recommendation:
            (loanData.credit_score || 0) > 700 ? "approved" : "needs_review",
        ...
        officerComments: Array.isArray(loanData.officer_comments)
          ? loanData.officer_comments
          : [],
  ...
  const handleProceedToScoring = async () => {
    try {
      ...
      setApplication((prev) =>
        prev ? { ...prev, status: "under_credit_scoring" } : null,
      );
      ...
      toast({
        ...
        description:
          "The model is analyzing the data. Results will appear shortly.",
      });
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      setApplication((prev) =>
        prev ? { ...prev, officerComments: updatedComments } : null,
      );
      setComment("");
  ...
  const saveEdit = (field: string) => {
    ...
    toast({
      title: "Field Updated",
      description: "Value updated locally. Submit to save to DB.",
    });
  };
  ...
  const handleExportReport = async () => {
    ...
    doc.text(
      `Monthly Income: Rs. ${application.applicant.salary.toLocaleString("en-IN")}`,
      14,
      72,
    );
    ...
    doc.setTextColor(50, 50, 50);
    doc.text(
      `Requested Amount: Rs. ${application.loan.amount.toLocaleString("en-IN")}`,
      120,
      48,
    );
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      `AI Credit Score: ${application.creditScore.score} / 900`,
      14,
      currentY,
    );
    currentY += 6;
    doc.text(
      `AI Recommendation: ${application.creditScore.recommendation.replace("_", " ").toUpperCase()}`,
      14,
      currentY,
    );
    currentY += 6;
    ...
    if (application.officerDecision) {
      doc.setFont("helvetica", "bold");
      doc.text(
        `Final Officer Decision: ${application.officerDecision.toUpperCase()}`,
        14,
        currentY,
      );
      doc.setFont("helvetica", "normal");
  ...
  const currentChartData =
    xaiMode === "SHAP"
      ? application?.creditScore.shapData
      : application?.creditScore.limeData;

  // Calculate comparison data for the modal and PDF
  const comparisonData =
    application?.creditScore.shapData.map((s) => {
      const l = application.creditScore.limeData.find(
        (lx) => lx.feature === s.feature,
      );
      return {
        feature: s.feature,
        SHAP: s.impact,
  ...
  if (!application)
    return <div className="text-center pt-20">Application not found</div>;
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                ID: {application.id ? application.id.slice(0, 8) + "..." : "ID"}
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            {application.officerDecision && (
              <Badge
                className={`text-md px-3 py-1 ${application.officerDecision === "approved" ? "bg-green-600" : "bg-red-600"}`}
              >
                {application.officerDecision === "approved" ? (
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="flex items-center">
              <FileText className="h-5 w-5 mr-2" /> Application Timeline
            </CardTitle>
          </CardHeader>
          <CardContent>
            <ApplicationTimeline
              stages={getApplicationTimeline(
              ...
              <TabsContent value="applicant">
                <div className="space-y-6">
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center">
                        <CreditCard className="h-5 w-5 mr-2" /> Loan Request
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-6">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Verify extracted values against documents
                      </p>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-center mb-6">
                      <div
                        className={`text-6xl font-bold ${getScoreColor(application.creditScore.score)}`}
                      >
                      ...
                      <div className="mt-4">
                        {application.creditScore.score > 0 ? (
                          application.creditScore.score > 700 ? (
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            ...
                            <DialogContent className="max-w-4xl">
                              <DialogHeader>
                                <DialogTitle>
                                  AI Model Comparison (SHAP vs LIME)
                                </DialogTitle>
                              </DialogHeader>
                              <div className="py-4">
                                <p className="text-sm text-muted-foreground mb-6">
                                  This chart compares the global feature impact
                                  (SHAP) against your local linear approximation
                                  (LIME).
                                </p>

                                {comparisonData &&
                                comparisonData.length > 0 &&
                                comparisonData.some((d) => d.LIME !== 0) ? (
                                  <ResponsiveContainer
                                    width="100%"
                                    height={500}
                                  >
                                  ...
                                  <div className="h-[400px] flex items-center justify-center border-2 border-dashed rounded-lg text-muted-foreground">
                                    Comparison data is incomplete. Please
                                    request a new scoring run.
                                  </div>
                                )}
                              </div>
                            </DialogContent>
                      ...
                      {currentChartData && currentChartData.length > 0 ? (
                        ...
                        <div className="h-[400px] flex items-center justify-center border-2 border-dashed rounded-lg text-muted-foreground">
                          No {xaiMode} data available. Please re-run credit
                          scoring.
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bg-muted p-4 rounded-lg border border-border">
                      <h4 className="font-bold mb-2 flex items-center">
                        <Cpu className="w-4 h-4 mr-2 text-primary" /> AI
                        Explanation
                      </h4>
                    ...
                    {application.creditScore.counterfactuals &&
                      application.creditScore.counterfactuals.length > 0 && (
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800 mt-4">
                          <h4 className="font-bold mb-2 flex items-center text-blue-700 dark:text-blue-400">
                            <Target className="w-4 h-4 mr-2" /> Suggestions to
                            Improve the Score (Counterfactuals)
                          </h4>
                          <div className="space-y-2 mt-3">
                            {application.creditScore.counterfactuals.map(
                              (cf, idx) => (
                                <div
                                  key={idx}
                                  className="text-sm p-3 bg-white dark:bg-slate-900 rounded-md border border-blue-100 dark:border-blue-800/50 shadow-sm"
                                >
            ...
            <Card className="sticky top-4">
              ...
              <CardContent className="space-y-4">
                <Button
                  ...
                  disabled={
                    application.status === "under_credit_scoring" ||
                    !!application.officerDecision
                  }
                >
                ...
                <Dialog
                  ...
                  onOpenChange={setIsClarifyModalOpen}
                >
                  <DialogTrigger asChild>
                    <Button
                      variant="outline"
                ...
                <div className="grid grid-cols-2 gap-2 pt-4 border-t">
                  ...
                  >
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                        <p className="text-sm text-muted-foreground">
                          Are you sure you want to finalize this loan as{" "}
                          <strong>APPROVED</strong>? This action will be logged.
                        </p>
                  ...
                  >
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                        <p className="text-sm text-muted-foreground">
                          Are you sure you want to finalize this loan as{" "}
                          <strong>REJECTED</strong>? This action cannot be
                          undone.
                        </p>
            ...
            >
              ...
              >
                ...
                <YAxis
                  ...
                  type="category"
                  width={200}
                  tick={{ fontSize: 12, fill: "#0f172a", fontWeight: "bold" }}
                />
                <Legend />
